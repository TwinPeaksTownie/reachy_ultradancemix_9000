"""
Bluetooth Streamer - Real-time audio-to-motion dance mode.
...
class BluetoothStreamer(DanceMode):
    """Bluetooth Streamer - Real-time audio streaming mode.

    Maps audio frequencies to robot movement with minimal latency.
    """

    MODE_ID = "bluetooth_streamer"
    MODE_NAME = "Bluetooth Streamer"
    ...
    async def start(self) -> None:
        """Start the bluetooth streamer."""
        if self.running:
            return

        # Initialize audio
        self.audio_stream = AudioStream.create_for_mode("bluetooth_streamer", AUDIO_CONFIG["sample_rate"])
